...
export interface Contact {
  ...
  /** WhatsApp @handle (migration 040) — the durable identity signal
   *  for a customer who has hidden their phone number via Meta's
   *  "WhatsApp usernames" feature. Editable by staff to manually link
   *  a known username onto an existing (phone-having) contact. */
  wa_username?: string | null;
  /** Current/last-known BSUID routing token (migration 040) — a
   *  per-business messaging credential that can rotate over time, not
   *  a stable identity. Read-only; refreshed automatically by the
   *  inbound webhook. */
